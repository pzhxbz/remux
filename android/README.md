...
1. 填写 Relay 的 `ws(s)://host:port`、Client token 和本机显示名；
2. 用系统文件选择器导入 Agent 生成的 `pairing.toml`；
3. 在机器页进入目标机器，管理或新建 session，然后点 Attach；
4. terminal 顶部可切换“历史手势/应用手势”，底部快捷栏可横向滚动。

模拟器访问宿主机 Relay 时可使用 `ws://10.0.2.2:<port>`。真实设备应使用可达的 WSS 域名；不要把 Relay token 或 pairing 文件提交到版本库。
